...
public class BinanceApiRestClientImpl implements BinanceApiRestClient {
    ...
    @Override
    public void marginTransfer(MarginTransferRequestModel marginTransfer) {
        executeSync(binanceApiService.marginTransfer(
                marginTransfer.getAsset(),
                marginTransfer.getAmount(),
                marginTransfer.getType(),
                marginTransfer.getRecvWindow(),
                marginTransfer.getTimestamp()
        ));
    }

    @Override
    public void marginLoan(MarginBorrowRequestModel marginTransfer) {
        executeSync(binanceApiService.marginLoan(
                marginTransfer.getAsset(),
                marginTransfer.getAmount(),
                marginTransfer.getRecvWindow(),
                marginTransfer.getTimestamp()
    ...
    @Override
    public void marginRepay(MarginBorrowRequestModel marginTransfer) {
        executeSync(binanceApiService.marginRepay(
                marginTransfer.getAsset(),
                marginTransfer.getAmount(),
                marginTransfer.getRecvWindow(),
                marginTransfer.getTimestamp()
    ...
    @Override
    public NewOrderResponse marginOrder(NewOrder order) {
        return executeSync(binanceApiService.marginOrder(order.getSymbol(), order.getSide(), order.getType(),
                                                         order.getTimeInForce(), order.getQuantity(), order.getPrice(), order.getNewClientOrderId(), order.getStopPrice(),
                                                         order.getIcebergQty(), order.getNewOrderRespType(), order.getRecvWindow(), order.getTimestamp()));
    }

    @Override
    public Order getMarginOrder(String symbol, String clientGeneratedId) {
        return executeSync(binanceApiService.getMarginOrder(
                symbol,
                clientGeneratedId,
    ...
    @Override
    public CancelOrderResponse marginCancelOrder(CancelOrderRequest cancelOrderRequest) {
        return executeSync(binanceApiService.marginCancelOrder(cancelOrderRequest.getSymbol(),
                                                               cancelOrderRequest.getOrderId(), cancelOrderRequest.getOrigClientOrderId(), cancelOrderRequest.getNewClientOrderId(),
                                                               cancelOrderRequest.getRecvWindow(), cancelOrderRequest.getTimestamp()));
    }
    ...
    public MarginAccount getMarginAccount(Long recvWindow, Long timestamp) {
        return executeSync(binanceApiService.getMarginAccountDetail(recvWindow, timestamp));
    }
